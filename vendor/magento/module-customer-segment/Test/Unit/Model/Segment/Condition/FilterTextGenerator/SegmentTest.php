...
class SegmentTest extends \PHPUnit_Framework_TestCase
{
    ...
    protected function setUp()
    {
        ...
        $this->segmentHelper = $this->getMock($className, ['isEnabled'], [], '', false);

        $className = '\Magento\CustomerSegment\Model\Customer';
        $this->segmentCustomer = $this->getMock($className, ['getCurrentCustomerSegmentIds'], [], '', false);

        $className = '\Magento\CustomerSegment\Model\Segment\Condition\FilterTextGenerator\Segment';
    ...
    public function testGenerateFilterText()
    {
        ...
        $this->segmentCustomer->expects($this->any())->method('getCurrentCustomerSegmentIds')->willReturn($segmentIds);
        /** @var \Magento\Quote\Model\Quote\Address|\PHPUnit_Framework_MockObject_MockObject $quoteAddress */
        $quoteAddress = $this->buildQuoteAddress();
    ...
    protected function buildQuoteAddress()
    {
        ...
        $quoteAddress = $this->getMock($className, [], [], '', false);
        return $quoteAddress;
    }
}
