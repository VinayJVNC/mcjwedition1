...
use Magento\MultipleWishlist\Controller\Adminhtml\Report\Customer\Wishlist\ExportExcel;
...
class ExportExcelTest extends \PHPUnit_Framework_TestCase
{
    ...
    protected function setUp()
    {
        ...
        $this->controller = new ExportExcel($this->context, $this->fileFactory);
    }
    ...
    public function testExecute()
    {
        ...
        $this->exportGridBlock->expects($this->once())
            ->method('getExcelFile')
            ->with('customer_wishlists.xml')
            ->willReturn('xmlFile');
        $this->fileFactory->expects($this->once())
            ->method('create')
            ->with('customer_wishlists.xml', 'xmlFile', DirectoryList::VAR_DIR)
