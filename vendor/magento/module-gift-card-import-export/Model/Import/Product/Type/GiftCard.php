...
class GiftCard extends \Magento\CatalogImportExport\Model\Import\Product\Type\AbstractType
{
    ...
    public function isRowValid(array $rowData, $rowNum, $isNewProduct = true)
    {
        ...
        if (!$this->isAmountValid($rowData)) {
            $this->_entityModel->addRowError(self::ERROR_AMOUNT_NOT_FOUND, $rowNum);
            $hasError = true;
        }
        return !$hasError;
    }
    ...
    public function prepareAttributesWithDefaultValueForSave(array $rowData, $withDefaultValue = true)
    {
        ...
        $resultAttributes = array_merge(
            ...
            $this->retrieveEmailTemplateId($rowData)
        );
    ...
    protected function insertAmounts()
    {
        ...
        $this->_connection->insertOnDuplicate($amountTable, $this->amountsCache);
        return $this;
    }
    ...
    {
        $result = [];
        if ($rowData[self::GIFTCARD_TYPE_COLUMN] == self::GIFTCARD_TYPE_VIRTUAL) {
            $result['weight'] = null;
        }
        return $result;
    }
