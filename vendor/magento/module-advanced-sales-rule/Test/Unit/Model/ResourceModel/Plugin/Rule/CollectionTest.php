...
class CollectionTest extends \PHPUnit_Framework_TestCase
{
    ...
    protected function setUp()
    {
        ...
        $this->ruleCollectionMock = $this->getMockBuilder('\Magento\SalesRule\Model\ResourceModel\Rule\Collection')
        ...
        $filterResourceFactoryMock = $this->getMockBuilder(
            'Magento\AdvancedSalesRule\Model\ResourceModel\Rule\Condition\FilterFactory'
        )->disableOriginalConstructor()
            ->setMethods(['create'])
            ->getMock();
        ...
        $this->filterTextGeneratorFactoryMock = $this->getMockBuilder(
            'Magento\AdvancedRule\Model\Condition\FilterTextGeneratorFactory'
        )->disableOriginalConstructor()
            ->getMock();

        $this->model = $this->objectManager->getObject(
            '\Magento\AdvancedSalesRule\Model\ResourceModel\Plugin\Rule\Collection',
            [
    ...
    public function testAroundSetValidationFilter()
    {
        ...
        $selectMock = $this->getMock('\Magento\Framework\DB\Select', null, [$connectionMock, $selectRendererMock]);
        ...
        $this->model->aroundSetValidationFilter(
        ...
        $this->assertEquals(1, count($selectMock->getPart(\Magento\Framework\DB\Select::WHERE)));
    }
    ...
    private function setupFilterResourceMock($addressMock, $filteredRuleIds)
    {
        ...
        $filterTextGenerator1 = $this->getMock(
        ...
        $filterTextGenerator2 = $this->getMock(
            '\Magento\AdvancedRule\Model\Condition\FilterTextGeneratorInterface'
        );
