...
class CombineTest extends \PHPUnit_Framework_TestCase
{
    ...
    protected function setUp()
    {
        ...
        $className = '\Magento\Framework\Event\ManagerInterface';
        $this->eventManager = $this->getMock($className, [], [], '', false);

        $className = '\Magento\SalesRule\Model\Rule\Condition\Address';
        $this->conditionAddress = $this->getMock($className, [], [], '', false);

        $className = '\Magento\AdvancedRule\Helper\CombineCondition';
        $this->conditionHelper = $this->getMock($className, [], [], '', false);

        $this->model = $this->objectManager->getObject(
            'Magento\AdvancedSalesRule\Model\Rule\Condition\Combine',
            [
