...
class UpdateTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function execute($arguments = [], $entityData = [], $isExceptionTest = true)
    {
        $rowId = isset($entityData['row_id']) ? $entityData['row_id'] : null;
        $rollBackId = isset($entityData['rollback_id']) ? $entityData['rollback_id'] : null;
        $identifier = isset($entityData['id']) ? $entityData['id'] : null;
        ...
        $versionInfo->expects($this->any())
            ->method('getCreatedIn')
            ->willReturn(isset($arguments['created_in']) ? $arguments['created_in'] : null);
        $versionInfo->expects($this->any())
            ->method('getUpdatedIn')
            ->willReturn(isset($arguments['updated_in']) ? $arguments['updated_in'] : null);
        ...
        $this->adapter->expects($this->once())
            ...
            ->willReturnSelf();
        $this->adapter->expects($isExceptionTest ? $this->once() : $this->never())
            ->method('rollBack')
            ->willReturnSelf();
        $this->metadataPool->expects($this->once())
            ->method('getHydrator')
        ...
        if ($isExceptionTest) {
            return $this->updateOperation->execute($this->entity);
        }
     ...
     */
    public function testExecute(array $arguments = [], array $entityData = [])
    {
        $this->execute($arguments, $entityData, false);
    }
}
