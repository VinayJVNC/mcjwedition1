...
class RescheduleUpdate
{
     ...
     * @return int
     */
    private function getPrevious(EntityMetadataInterface $metadata, $version, $identifier)
    {
     ...
     * @return int
     */
    private function getNext(EntityMetadataInterface $metadata, $version, $identifier)
    {
        $connection = $this->resourceConnection->getConnectionByName($metadata->getEntityConnectionName());
        $select = $connection->select()
            ->from(
    ...
    {
        $connection = $this->resourceConnection->getConnectionByName($metadata->getEntityConnectionName());
        $previous = $this->getPrevious($metadata, $originVersion, $identifier);
        $next = $this->getNext($metadata, $originVersion, $identifier);
        $updatedIn = $targetVersion > $next ? $next : $targetVersion;
        $connection->update(
    ...
    {
        $connection = $this->resourceConnection->getConnectionByName($metadata->getEntityConnectionName());
        $previous = $this->getPrevious($metadata, $targetVersion, $identifier);
        if ($previous != $originVersion) {
            $connection->update(
                $metadata->getEntityTable(),
    ...
    private function updateEntry(
        ...
        if ($target->getRollbackId()) {
            ...
            $updateIn = $this->getNext($metadata, $target->getId(), $identifier);
        }
