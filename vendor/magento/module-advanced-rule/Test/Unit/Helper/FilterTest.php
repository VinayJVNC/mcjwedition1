...
class FilterTest extends \PHPUnit_Framework_TestCase
{
    ...
    protected function setUp()
    {
        ...
        $this->filterInterfaceFactoryMock = $this->getMockBuilder(
            'Magento\AdvancedRule\Model\Condition\FilterInterfaceFactory'
        ...
        $this->filterInterfaceFactoryMock->expects($this->any())
            ...
            ->willReturnCallback(function () use ($objManager) {
                return $objManager->getObject('Magento\AdvancedRule\Model\Condition\Filter');
            });

        $this->filterGroupInterfaceFactoryMock = $this->getMockBuilder(
            'Magento\AdvancedRule\Model\Condition\FilterGroupInterfaceFactory'
        )->disableOriginalConstructor()
            ->setMethods(['create'])
            ->getMock();
        $this->filterGroupInterfaceFactoryMock->expects($this->any())
            ->method('create')
            ->willReturnCallback(function () use ($objManager) {
                return $objManager->getObject('Magento\AdvancedRule\Model\Condition\FilterGroup');
            });

        $this->filterHelper = $this->objectManager->getObject(
            '\Magento\AdvancedRule\Helper\Filter',
            [
    ...
    protected function setupFilterGroup($filters)
    {
        ...
        $filterGroup = $this->objectManager->getObject('Magento\AdvancedRule\Model\Condition\FilterGroup');
    ...
    protected function setupFilter(array $filterData)
    {
        $filter = $this->objectManager->getObject(
            'Magento\AdvancedRule\Model\Condition\Filter',
            [
                'data' => $filterData
            ]
        );
