...
class CombineConditionTest extends \PHPUnit_Framework_TestCase
{
    /**
    ...
    protected function setUp()
    {
        ...
        $this->filterHelperMock = $this->getMockBuilder('\Magento\AdvancedRule\Helper\Filter')
            ->disableOriginalConstructor()
            ->getMock();

        $this->combineConditionHelper = $this->objectManager->getObject(
            '\Magento\AdvancedRule\Helper\CombineCondition',
            [
    ...
    {
        return $this->getMock('Magento\AdvancedRule\Model\Condition\FilterGroupInterface');
    }

    protected function getFilterableConditionMock($isFilterable, $filterGroups = null)
    {
        $mock = $this->getMock('\Magento\AdvancedRule\Model\Condition\FilterableConditionInterface');
