...
class EditTest extends \PHPUnit_Framework_TestCase
{
    ...
    protected function setUp()
    {
        ...
        $this->contextMock->expects($this->atLeastOnce())
            ...
            ->willReturn($this->getMock('Magento\Framework\App\ActionFlag', [], [], '', false));
        $this->contextMock->expects($this->atLeastOnce())
            ->method('getHelper')
            ->willReturn($this->getMock('Magento\Backend\Helper\Data', [], [], '', false));
        $this->contextMock->expects($this->atLeastOnce())
            ->method('getView')
            ->willReturn($this->viewMock);
        $this->action = $this->objectManager->getObject(
            'Magento\Rma\Controller\Adminhtml\Rma\Item\Attribute\Edit',
            [
    ...
    public function mockInitAttribute($website = null)
    {
        ...
        $this->objectManagerMock->expects($this->once())
            ->method('create')
            ->with('Magento\Rma\Model\Item\Attribute', [])
